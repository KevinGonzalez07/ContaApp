...
function Spinner() {
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: 200 }}>
      <div style={{ animation: "spin 1s linear infinite" }}><Icon name="spinner" size={32} color={C.primary} /></div>
...
function exportProcessesPDF(processes, clients, despacho, month, year) {
  ...
  doc.setFontSize(10); doc.setTextColor(107, 107, 103); doc.text(`${despacho || "Despacho Contable"} · Generado: ${new Date().toLocaleDateString("es-MX")}`, 14, 28);
  autoTable(doc, { startY: 35, head: [["Proceso", "Cliente", "Tipo", "Vencimiento", "Estado"]], body: filtered.map(p => [p.title, clientMap[p.client_id]?.name || "—", PROCESS_TYPES[p.type], p.due_date, p.status]), headStyles: { fillColor: [27, 58, 92], textColor: 255, fontSize: 10 }, bodyStyles: { fontSize: 9 }, alternateRowStyles: { fillColor: [247, 246, 242] } });
  doc.save(`procesos_${MONTHS[month]}_${year}.pdf`);
}
function exportProcessesExcel(processes, clients, month, year) {
  const clientMap = Object.fromEntries(clients.map(c => [c.id, c]));
  const filtered = processes.filter(p => { const d = new Date(p.due_date); return d.getMonth() === month && d.getFullYear() === year; });
  const ws = XLSX.utils.json_to_sheet(filtered.map(p => ({ Proceso: p.title, Cliente: clientMap[p.client_id]?.name || "—", Tipo: PROCESS_TYPES[p.type], Inicio: p.start_date, Vencimiento: p.due_date, Estado: p.status, Notas: p.notes || "" })));
...
function GlobalSearch({ clients, processes, setActive, onClose }) {
  ...
  const results = useMemo(() => {
    ...
    const matchedClients = clients.filter(c => c.name.toLowerCase().includes(term) || c.rfc?.toLowerCase().includes(term) || c.email?.toLowerCase().includes(term)).map(c => ({ type: "client", label: c.name, sub: c.rfc || c.email, id: c.id }));
    const matchedProcesses = processes.filter(p => p.title.toLowerCase().includes(term) || p.notes?.toLowerCase().includes(term)).map(p => ({ type: "process", label: p.title, sub: PROCESS_TYPES[p.type] + " · " + p.due_date, id: p.id, status: p.status }));
    return [...matchedClients.slice(0, 4), ...matchedProcesses.slice(0, 4)];
  }, [q, clients, processes]);
      ...
      <div style={{ background: C.cardBg, borderRadius: 14, width: "100%", maxWidth: 560, boxShadow: "0 12px 48px rgba(0,0,0,0.35)", border: `1px solid ${C.border}` }}>
        ...
        {q && (
          <div style={{ maxHeight: 360, overflow: "auto" }}>
            {results.length === 0 ? (
              <p style={{ padding: "24px", textAlign: "center", color: C.textMuted, fontSize: 14 }}>Sin resultados para "{q}"</p>
            ) : results.map((r, i) => (
              <button key={i} onClick={() => { setActive(r.type === "client" ? "clients" : "processes"); onClose(); }} style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "12px 20px", background: "none", border: "none", cursor: "pointer", borderBottom: i < results.length - 1 ? `1px solid ${C.border}` : "none", textAlign: "left" }}>
                <div style={{ width: 34, height: 34, borderRadius: 8, background: C.primary + "20", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <Icon name={r.type === "client" ? "user" : "processes"} size={16} color={C.primary} />
                </div>
        ...
        {!q && <p style={{ padding: "20px 24px", fontSize: 13, color: C.textMuted }}>Escribe para buscar en clientes y procesos · Presiona Esc para cerrar</p>}
      </div>
    </div>
  );
}
  ...
  return (
    <Modal title="Mi perfil" onClose={onClose}>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24, padding: 16, background: C.surface, borderRadius: 10 }}>
        <div style={{ width: 52, height: 52, borderRadius: "50%", background: C.primary + "30", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 700, color: C.primary }}>
          {(profile?.name || "U").split(" ").slice(0, 2).map(n => n[0]).join("")}
        </div>
        <div>
          <p style={{ margin: "0 0 2px", fontSize: 16, fontWeight: 600, color: C.text }}>{profile?.name}</p>
          <p style={{ margin: 0, fontSize: 13, color: C.textMuted }}>{profile?.despacho}</p>
        </div>
      </div>
      ...
      <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 16, marginTop: 4 }}>
        ...
        <FormField label="Confirmar contraseña">
          <input style={inputStyle} type={showPass ? "text" : "password"} value={form.confirmPassword} onChange={e => setForm(f => ({ ...f, confirmPassword: e.target.value }))} placeholder="Repite la contraseña" />
        </FormField>
      </div>
...
function AuthScreen({ onAuth }) {
  const C = getColors(false);
  const inputStyle = { width: "100%", padding: "9px 12px", border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 14, color: C.text, background: C.white, outline: "none", boxSizing: "border-box" };
      ...
      <div style={{ display: "flex", width: "100%", maxWidth: 900, background: C.white, borderRadius: 16, overflow: "hidden", boxShadow: "0 4px 40px rgba(0,0,0,0.1)" }}>
        <div style={{ flex: 1, background: C.primary, padding: "60px 48px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.15)", paddingTop: 32 }}>
            {["Gestión de clientes y RFC", "Control de declaraciones", "Exportar PDF y Excel", "Calendario de vencimientos", "Modo oscuro incluido"].map(f => (
        ...
        <div style={{ flex: 1, padding: "60px 48px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          ...
          {mode === "register" && <>
            <div style={{ marginBottom: 16 }}><label style={{ display: "block", fontSize: 13, fontWeight: 500, color: C.textMuted, marginBottom: 6 }}>Nombre completo <span style={{ color: C.danger }}>*</span></label><input style={inputStyle} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Lic. María González" /></div>
            <div style={{ marginBottom: 16 }}><label style={{ display: "block", fontSize: 13, fontWeight: 500, color: C.textMuted, marginBottom: 6 }}>Nombre del despacho <span style={{ color: C.danger }}>*</span></label><input style={inputStyle} value={form.despacho} onChange={e => setForm(f => ({ ...f, despacho: e.target.value }))} placeholder="Despacho Contable González" /></div>
          </>}
          <div style={{ marginBottom: 16 }}><label style={{ display: "block", fontSize: 13, fontWeight: 500, color: C.textMuted, marginBottom: 6 }}>Correo electrónico <span style={{ color: C.danger }}>*</span></label><input style={inputStyle} type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="correo@ejemplo.mx" onKeyDown={e => e.key === "Enter" && handleSubmit()} /></div>
          <div style={{ marginBottom: 16 }}><label style={{ display: "block", fontSize: 13, fontWeight: 500, color: C.textMuted, marginBottom: 6 }}>Contraseña <span style={{ color: C.danger }}>*</span></label><input style={inputStyle} type="password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} placeholder="Mínimo 6 caracteres" onKeyDown={e => e.key === "Enter" && handleSubmit()} /></div>
...
function Sidebar({ active, setActive, profile, onLogout, dark, setDark, onOpenProfile, onOpenSearch }) {
  ...
  const sidebarBg = dark ? "#0F1923" : "#1B3A5C";
  return (
    <div style={{ width: 240, background: sidebarBg, display: "flex", flexDirection: "column", minHeight: "100vh", flexShrink: 0 }}>
      ...
      <div style={{ padding: "12px 12px 20px", borderTop: "1px solid rgba(255,255,255,0.1)" }}>
        <button onClick={onOpenProfile} style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "10px 12px", borderRadius: 8, border: "none", cursor: "pointer", background: "none", marginBottom: 4 }}>
          <div style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(255,255,255,0.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 600, color: "#fff", flexShrink: 0 }}>
            {(profile?.name || "U").split(" ").slice(0, 2).map(n => n[0]).join("")}
          </div>
        ...
        <button onClick={() => setDark(d => !d)} style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "8px 12px", borderRadius: 8, border: "none", cursor: "pointer", background: "none", color: "rgba(255,255,255,0.5)", fontSize: 13, marginBottom: 2 }}>
          <Icon name={dark ? "sun" : "moon"} size={16} color="rgba(255,255,255,0.5)" />
...
function StatCard({ label, value, icon, color, sub }) {
  ...
  return (
    <div style={{ background: C.cardBg, borderRadius: 12, padding: "20px 22px", border: `1px solid ${C.border}`, display: "flex", alignItems: "flex-start", gap: 16 }}>
      ...
      <div>
        <p style={{ margin: "0 0 4px", fontSize: 13, color: C.textMuted }}>{label}</p>
        <p style={{ margin: "0 0 2px", fontSize: 26, fontWeight: 700, color: C.text, lineHeight: 1 }}>{value}</p>
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        ...
        <div style={{ background: C.cardBg, borderRadius: 12, border: `1px solid ${C.border}`, overflow: "hidden" }}>
          <div style={{ padding: "16px 20px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 8 }}><Icon name="clock" size={16} color={C.warning} /><h3 style={{ margin: 0, fontSize: 15, fontWeight: 600, color: C.text }}>Próximos vencimientos</h3></div>
          {dueSoon.length === 0 ? <p style={{ padding: "20px", color: C.textMuted, fontSize: 14, textAlign: "center" }}>Sin vencimientos próximos</p>
            : dueSoon.slice(0, 5).map(p => (
              <div key={p.id} style={{ padding: "12px 20px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div><p style={{ margin: "0 0 2px", fontSize: 14, fontWeight: 500, color: C.text }}>{p.title}</p><p style={{ margin: 0, fontSize: 12, color: C.textMuted }}>{clientMap[p.client_id]?.name}</p></div>
                <span style={{ background: C.warningLight, color: C.warning, fontSize: 12, padding: "3px 8px", borderRadius: 6 }}>{daysLeft(p.due_date) === 0 ? "Hoy" : `${daysLeft(p.due_date)}d`}</span>
              </div>
            ))}
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        ...
        <div style={{ background: C.cardBg, borderRadius: 12, border: `1px solid ${C.border}`, padding: 24 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
            <div style={{ marginBottom: 0 }}><label style={{ display: "block", fontSize: 13, fontWeight: 500, color: C.textMuted, marginBottom: 6 }}>Mes</label><select style={inputStyle} value={reportMonth} onChange={e => setReportMonth(Number(e.target.value))}>{MONTHS.map((m, i) => <option key={i} value={i}>{m}</option>)}</select></div>
            <div style={{ marginBottom: 0 }}><label style={{ display: "block", fontSize: 13, fontWeight: 500, color: C.textMuted, marginBottom: 6 }}>Año</label><select style={inputStyle} value={reportYear} onChange={e => setReportYear(Number(e.target.value))}>{years.map(y => <option key={y} value={y}>{y}</option>)}</select></div>
          </div>
          <div style={{ background: C.surface, borderRadius: 8, padding: 12, marginBottom: 16 }}>
          ...
          <tbody>
            {filtered.map((c, i) => (
              <tr key={c.id} style={{ borderBottom: i < filtered.length - 1 ? `1px solid ${C.border}` : "none" }}>
                ...
                <td style={{ padding: "14px 16px" }}><span style={{ background: c.type === "moral" ? "#0D2A3D" : C.accentLight, color: c.type === "moral" ? "#64B5F6" : "#0F6E56", fontSize: 12, padding: "3px 10px", borderRadius: 20, fontWeight: 500 }}>{c.type === "fisica" ? "Física" : "Moral"}</span></td>
            ...
            return (
              <div key={p.id} style={{ padding: "14px 20px", borderBottom: i < upcoming.length - 1 ? `1px solid ${C.border}` : "none", background: isOverdue ? C.dangerLight : isUrgent ? C.warningLight : "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ display: "flex", gap: 12 }}>
                  ...
                  <div><p style={{ margin: "0 0 2px", fontSize: 14, fontWeight: 500, color: C.text }}>{p.title}</p><p style={{ margin: 0, fontSize: 12, color: C.textMuted }}>{clientMap[p.client_id]?.name} · {PROCESS_TYPES[p.type]} · {p.reminder || 3}d antes</p></div>
                </div>
...
export default function App() {
  ...
  const [dark, setDark] = useState(() => localStorage.getItem("conta_dark") === "true");
  const [showSearch, setShowSearch] = useState(false);
  const [showProfile, setShowProfile] = useState(false);
  const C = getColors(dark);

  useEffect(() => { localStorage.setItem("conta_dark", dark); }, [dark]);
