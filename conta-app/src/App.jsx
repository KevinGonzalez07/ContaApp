...
async function generateReciboPDF(hon, client, profile) {
  ...
  for (let x = 0; x <= W; x += 8) doc.line(x, 0, x, 297);
  for (let y = 0; y <= 297; y += 8) doc.line(0, y, W, y);
  ...
  doc.roundedRect(M, 8, W - M * 2, 270, 4, 4, "F");
  doc.setDrawColor(220, 220, 220);
  doc.setLineWidth(0.3);
  doc.roundedRect(M, 8, W - M * 2, 270, 4, 4, "S");

  const cardL = M + 8, cardR = W - M - 8;
  ...
  if (profile?.logo_url) {
    try {
      ...
      doc.addImage(dataUrl, imgFmt, cardL, 16, 28, 28, undefined, "FAST");
      logoLoaded = true;
    } catch (e) { /* fallback */ }
  }
  if (!logoLoaded) {
    doc.setFillColor(29, 158, 117);
    doc.roundedRect(cardL, 16, 28, 28, 3, 3, "F");
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(13);
    doc.setFont("helvetica", "bold");
    const initials = (profile?.despacho || "CD").split(" ").slice(0, 2).map(w => w[0]).join("").toUpperCase();
    doc.text(initials, cardL + 14, 32, { align: "center" });
  }
  ...
  doc.setFontSize(15);
  doc.setFont("helvetica", "bold");
  doc.text(profile?.despacho || "Despacho Contable", cardL + 34, 24);
  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(100, 100, 100);
  if (profile?.email) doc.text(profile.email, cardL + 34, 30);

  // ── RECIBO badge top-right ──
  doc.setFillColor(27, 58, 92);
  doc.roundedRect(cardR - 44, 16, 44, 16, 3, 3, "F");
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(13);
  doc.setFont("helvetica", "bold");
  doc.text("RECIBO", cardR - 22, 26, { align: "center" });
  doc.setFontSize(8);
  doc.setFont("helvetica", "normal");
  doc.text(`No. ${String(hon.id).padStart(5, "0")}`, cardR - 22, 32, { align: "center" });

  // ── divider ──
  doc.setDrawColor(230, 230, 230);
  doc.setLineWidth(0.4);
  doc.line(cardL, 50, cardR, 50);

  // ── STATUS pill ──
  const statusBg = { pagado: [29, 158, 117], pendiente: [133, 79, 11], parcial: [24, 95, 165], vencido: [163, 45, 45] };
  const sc = statusBg[hon.status] || statusBg.pendiente;
  doc.setFillColor(...sc);
  doc.roundedRect(cardL, 54, 36, 8, 2, 2, "F");
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(8);
  doc.setFont("helvetica", "bold");
  doc.text((HON_STATUS[hon.status]?.label || "Pendiente").toUpperCase(), cardL + 18, 59.5, { align: "center" });

  // ── Two-column info block ──
  const col1X = cardL, col2X = cardL + 88;
  let y = 70;
  ...
  if (client?.rfc) { doc.text(`RFC: ${client.rfc}`, col1X, y); y += 4.5; }
  if (client?.email) { doc.text(client.email, col1X, y); y += 4.5; }
  if (client?.phone) { doc.text(client.phone, col1X, y); }

  // Right: dates info
  const dateRows = [
    ["Fecha de emisión", hon.fecha_emision || fmt(today)],
    ["Período", hon.periodo || "—"],
    ["Vencimiento", hon.fecha_vencimiento || "—"],
    ["Tipo", hon.tipo === "fijo" ? "Honorario fijo mensual" : "Por trámite realizado"],
  ];
  let dy = 75;
  dateRows.forEach(([label, value]) => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(130, 130, 130);
    doc.text(label.toUpperCase(), col2X, dy);
    doc.setFontSize(9.5);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(40, 40, 40);
    doc.text(String(value), col2X, dy + 4.5);
    dy += 12;
  });
  ...
  doc.roundedRect(cardL, tableY, cardR - cardL, 8, 1, 1, "F");
  doc.setFontSize(8);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(60, 80, 110);
  doc.text("CONCEPTO", cardL + 4, tableY + 5.5);
  doc.text("MONTO", cardR - 4, tableY + 5.5, { align: "right" });

  doc.setFillColor(255, 255, 255);
  doc.rect(cardL, tableY + 8, cardR - cardL, 14, "F");
  doc.setDrawColor(220, 225, 235);
  doc.setLineWidth(0.3);
  doc.rect(cardL, tableY + 8, cardR - cardL, 14, "S");
  doc.setFontSize(10);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(30, 30, 30);
  const conceptLines = doc.splitTextToSize(hon.concepto, cardR - cardL - 50);
  doc.text(conceptLines, cardL + 4, tableY + 16);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  doc.setTextColor(27, 58, 92);
  doc.text(fmtMoney(hon.monto), cardR - 4, tableY + 16, { align: "right" });

  // ── Payment summary ──
  const sumY = tableY + 30;
  const sumX = cardR - 78;
  doc.setFillColor(246, 248, 252);
  doc.roundedRect(sumX, sumY, 78, 42, 3, 3, "F");
  doc.setDrawColor(210, 218, 230);
  doc.setLineWidth(0.3);
  doc.roundedRect(sumX, sumY, 78, 42, 3, 3, "S");

  const rows = [
    ["Subtotal", fmtMoney(hon.monto), [60, 60, 60], false],
    ["Pagado", fmtMoney(hon.monto_pagado || 0), [29, 158, 117], false],
  ];
  const saldo = Number(hon.monto) - Number(hon.monto_pagado || 0);
  rows.forEach(([label, value, color, bold], idx) => {
    const ry = sumY + 9 + idx * 9;
    doc.setFontSize(8.5);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(110, 110, 110);
    doc.text(label, sumX + 6, ry);
    doc.setFont(bold ? "helvetica" : "helvetica", bold ? "bold" : "normal");
    doc.setTextColor(...color);
    doc.text(value, sumX + 72, ry, { align: "right" });
  });
  // Saldo divider
  doc.setDrawColor(210, 218, 230);
  doc.line(sumX + 6, sumY + 28, sumX + 72, sumY + 28);
  doc.setFontSize(9);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(60, 60, 60);
  doc.text("SALDO PENDIENTE", sumX + 6, sumY + 36);
  doc.setFontSize(12);
  doc.setTextColor(...(saldo <= 0 ? [29, 158, 117] : [163, 45, 45]));
  doc.text(fmtMoney(saldo), sumX + 72, sumY + 36, { align: "right" });

  // ── Notes ──
  if (hon.notas) {
    const notesY = sumY + 50;
    doc.setFillColor(255, 251, 240);
    doc.setDrawColor(240, 200, 120);
    doc.setLineWidth(0.3);
    doc.roundedRect(cardL, notesY, cardR - cardL, 20, 2, 2, "F");
    doc.roundedRect(cardL, notesY, cardR - cardL, 20, 2, 2, "S");
    doc.setFontSize(7.5);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(130, 90, 0);
    doc.text("NOTAS", cardL + 4, notesY + 6);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.setTextColor(60, 50, 10);
    const notesLines = doc.splitTextToSize(hon.notas, cardR - cardL - 12);
    doc.text(notesLines, cardL + 4, notesY + 12);
  }
  ...
  doc.setLineWidth(0.4);
  doc.line(cardL, sigY, cardL + 60, sigY);
  doc.setFontSize(8);
  doc.setTextColor(150, 150, 150);
  doc.setFont("helvetica", "normal");
  doc.text("Firma del responsable", cardL, sigY + 5);
  doc.text(profile?.name || "", cardL, sigY + 10);
  ...
  doc.setFont("helvetica", "normal");
  doc.text(`${profile?.despacho || "Despacho Contable"} · Generado con ContaDesk · ${new Date().toLocaleDateString("es-MX")}`, W / 2, 291, { align: "center" });

  doc.save(`recibo_${String(hon.id).padStart(5, "0")}_${client?.name?.replace(/\s+/g, "_") || "cliente"}.pdf`);
}
...
function Icon({ name, size = 18, color = "currentColor" }) {
  const icons = {
    ...
    image: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.8"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21,15 16,10 5,21"/></svg>,
  };
...
// ─── PROFILE MODAL (with logo upload) ────────────────────────────────────────
function ProfileModal({ user, profile, onClose, onUpdate }) {
  ...
  const save = async () => {
    ...
    if (form.newPassword && form.newPassword.length < 6) { setError("Mínimo 6 caracteres para la contraseña."); return; }
    setSaving(true);
    try {
    ...
    <Modal title="Mi perfil" onClose={onClose} maxWidth={560}>
      ...
      <div style={{ background: C.surface, borderRadius: 12, padding: 20, marginBottom: 20, display: "flex", alignItems: "center", gap: 20 }}>
        <div style={{ position: "relative", flexShrink: 0 }}>
          {logoPreview ? (
            <img src={logoPreview} alt="Logo" style={{ width: 72, height: 72, borderRadius: 10, objectFit: "contain", border: `1px solid ${C.border}`, background: "#fff", padding: 4 }} />
          ) : (
            <div style={{ width: 72, height: 72, borderRadius: 10, background: C.primary + "25", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 700, color: C.primary }}>
              {(profile?.despacho || "D").split(" ").slice(0, 2).map(w => w[0]).join("").toUpperCase()}
            </div>
          )}
        </div>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => fileRef.current?.click()} style={{ ...btnSecondary, padding: "7px 14px", fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>
              <Icon name="upload" size={14} color={C.gray} /> {logoPreview ? "Cambiar logo" : "Subir logo"}
            </button>
          ...
          <input ref={fileRef} type="file" accept="image/png,image/jpeg,image/jpg" style={{ display: "none" }} onChange={handleLogoChange} />
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
        ...
        <button onClick={save} disabled={saving || uploadingLogo} style={{ ...btnPrimary, opacity: (saving || uploadingLogo) ? 0.7 : 1 }}>{saving ? "Guardando..." : uploadingLogo ? "Subiendo logo..." : "Guardar cambios"}</button>
      </div>
    </Modal>
  );
}
...
function GlobalSearch({ clients, processes, setActive, onClose }) {
  ...
  const results = useMemo(() => {
    ...
    const mp = processes.filter(p => p.title.toLowerCase().includes(term) || p.notes?.toLowerCase().includes(term)).map(p => ({ type: "process", label: p.title, sub: PROCESS_TYPES[p.type] + " · " + p.due_date }));
      ...
      <div style={{ display: "flex", width: "100%", maxWidth: 900, background: C.white, borderRadius: 16, overflow: "hidden", boxShadow: "0 4px 40px rgba(0,0,0,0.1)" }}>
        <div style={{ flex: 1, background: C.primary, padding: "60px 48px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.15)", paddingTop: 32 }}>
            {["Gestión de clientes y RFC", "Control de declaraciones", "Módulo de honorarios", "Recibos PDF con tu logo", "Calendario y exportación"].map(f => (
...
function HonorariosView({ honorarios, setHonorarios, clients, userId, profile }) {
  ...
  const save = async () => {
    if (!form.concepto || !form.monto || !form.client_id) return; setSaving(true);
  ...
  const handleGeneratePDF = async (h) => {
    setGeneratingPDF(h.id);
    await generateReciboPDF(h, clientMap[h.client_id], profile);
